...
def generate_detections(encoder, img_dir, detections_dir):
    ...
    """
    img_fpaths = glob(img_dir + "/*.png")
    for img_fpath in img_fpaths:
        frame_id = int(Path(img_fpath).name.strip('.png').split('_')[-1])
        img = cv2.imread(img_fpath, cv2.IMREAD_COLOR)
        det_fpath = Path(detections_dir, Path(img_fpath).name + '.processed.npy')
        det = np.load(det_fpath).reshape((1,))[0]
        rows = []
        for i in range(len(det['rois'])):
            label = det['labels'][i]
            if label in ['red_team','blue_team']:
                y1,x1,y2,x2 = det['rois'][i]
                w = x2 - x1
                h = y2 - y1
                score = det['scores'][i]
                rows.append([int(frame_id), -1, x1, y1, w, h, 1, -1, -1, -1])
        rows = np.asarray(rows)
        features = encoder(img, rows[:, 2:6].copy())
        detections_out = []
        detections_out += [np.r_[(row, feature)] for row, feature
                               in zip(rows, features)]
        output_fpath = os.path.join(detections_dir, "%s.deepsort.npy" % Path(img_fpath).name)
        np.save(output_fpath, np.asarray(detections_out), allow_pickle=False)
...
def parse_args():
    ...
    parser.add_argument(
        "--model",
        default="data/volleyball/mars-small128.ckpt-68577",
        help="Path to checkpoint file")
    parser.add_argument(
        "--loss_mode", default="cosine", help="Network loss training mode")
    parser.add_argument(
        "--img_dir", help="path to video frames", required=True)
    parser.add_argument(
        "--detections_dir", help="Path to custom detections.", required=True)
    return parser.parse_args()


if __name__ == "__main__":
    args = parse_args()
    encoder = create_box_encoder(args.model, batch_size=32, loss_mode=args.loss_mode)
#    generate_detections(f, args.mot_dir, args.output_dir, args.detection_dir)
    generate_detections(encoder, args.img_dir, args.detections_dir)
